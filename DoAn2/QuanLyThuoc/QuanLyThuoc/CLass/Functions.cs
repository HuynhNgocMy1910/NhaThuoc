...
namespace QuanLyThuoc.CLass
{
    internal class Functions
    {
        ...
        {
            Con = new SqlConnection("Data Source=DESKTOP-CKDHHJ1\\KHAVY;Initial Catalog=QLThuoc;Integrated Security=True");
            try
            {
                Con.Open();
            }
        ...
        public static string ChuyenSoSangChu(string sNumber)
        {
            ...
            for (int i = 0; i <= mLen; i++)
            {
                ...
                switch ((mLen - i) % 9)
                {
                    case 0:
                        mTemp = mTemp + " tỷ";
                        i = SkipZeros(sNumber, i, 3); // Bỏ qua các nhóm "000"
                        break;
                    case 6:
                        mTemp = mTemp + " triệu";
                        i = SkipZeros(sNumber, i, 3); // Bỏ qua các nhóm "000"
        ...
        /// Trả về chỉ số mới sau khi bỏ qua các nhóm "000", hoặc giữ nguyên chỉ số ban đầu nếu không có nhóm "000".
        private static int SkipZeros(string sNumber, int currentIndex, int groupSize)
        {
            int maxLength = sNumber.Length; // Độ dài tối đa của chuỗi
                                            // Kiểm tra nếu vị trí hiện tại + kích thước nhóm nằm trong giới hạn chuỗi
            while (currentIndex + groupSize < maxLength &&
                   sNumber.Substring(currentIndex + 1, groupSize) == new string('0', groupSize))
            {
                currentIndex += groupSize; // Bỏ qua nhóm "000" bằng cách tăng chỉ số
            }
            return currentIndex; // Trả về chỉ số cuối cùng sau khi xử lý
        }
    }
}
